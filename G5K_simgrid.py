...
# Creating the AS
platform = ET.Element('AS', attrib = {'id': 'grid5000.fr', 'routing': default_routing})    
for site in sites:
    ET.SubElement(platform, 'AS', attrib = {'id': site+suffix, 'routing': default_routing})
# Creating the backbone links
for element1, element2, attrib in sorted(gr.edges_iter(data=True)):    
    ET.SubElement(platform, 'link', attrib = {'id': element1+'_'+element2, 
...
for el in product(gws, gws):
    if el[0] != el[1]:
        p = platform.find("./ASroute/[@gw_src='"+el[1]+"'][@gw_dst='"+el[0]+"']")
        if p is None:
            asroute = ET.SubElement(platform, 'ASroute', attrib = {
...
for site, sgr in site_gr.iteritems():
    site_el = platform.find("./AS/[@id='"+site+"']")
...
platform = xml.dom.minidom.parseString(ET.tostring(platform))
